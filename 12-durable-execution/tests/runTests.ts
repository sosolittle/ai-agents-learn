...
/** A raw, JSON-serializable workflow record for hand-writing persisted state directly to disk. */
...
// ── two idempotency boundaries: workflow-start vs step-execution ───────────
...
// Different workflows (from different approvals) still get independent
// step-level idempotency keys. This is deliberately NOT the same approval
// submitted twice — that duplicate-start path is covered above and is a
...
// ── validate_approval business rules ────────────────────────────────────────
...
// ── completedSteps prefix invariant ─────────────────────────────────────────
...
test("the dangerous crash-window state remains a valid persisted workflow", () => {
  const paths = tempPaths();
  // status "running", only validate_approval checkpointed, empty context — but
  // a refund effect already independently exists in the ledger. This gap
  // (the provider knows before the checkpoint does) is exactly what the crash
  // demo relies on, so the schema must not reject it.
  writeFileSync(
    paths.workflows,
