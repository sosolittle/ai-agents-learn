...
当前示例先执行非流式调用，再执行流式调用。提示词使用中文并要求模型用两个简短段落回答；具体内容、数据块数量和字符数每次可能不同。
...
当前示例兼容通过 `OPENAI_BASE_URL` 配置 DeepSeek。DeepSeek V4 默认开启思考模式，思考过程位于 `reasoning_content`，最终正文位于 `content`。本课关注正文的流式输出，因此检测到 DeepSeek 地址时会发送 `thinking: { type: "disabled" }`，避免有限的 `max_tokens` 被思考过程消耗完后没有正文可供展示。

如果后续要学习推理模型的思考事件，可以开启思考模式，并分别拼接 `delta.reasoning_content` 与 `delta.content`。两者不能混为一条正文：前者是模型的中间推理，后者才是面向用户的最终回答。
...
- 把提示词中的“两个简短的段落”改成“四个简短的段落”，对比两种方式的等待感受。
